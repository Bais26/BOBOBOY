...
export default function TambahLokasiWFOPopup({ 
  ...
  const [validationResult, setValidationResult] = useState<GPSValidationResponse | null>(null)
  ...
  const testGPSValidation = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Terjadi kesalahan saat validasi GPS')
    } finally {
      setLoading(false)
    }
  }
  ...
  const activeLocations = existingLocations.filter(loc => loc.is_active !== false)
  const locationsToShow = showAllLocations ? activeLocations : activeLocations.slice(0, 3)
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <Dialog.Title as="h3" className="text-xl font-semibold text-gray-900">
                        {editingLocation?.id ? 'Edit Lokasi WFO' : 'Tambah Lokasi WFO Baru'}
                      </Dialog.Title>
                      <p className="text-sm text-gray-500 mt-1">
                        {editingLocation?.id ? 'Perbarui lokasi kantor' : 'Atur lokasi untuk Work From Office'}
                      </p>
                    </div>
                    <button
                      type="button"
                ...
                <div className="max-h-[70vh] overflow-y-auto p-6 modal-content">
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-blue-600 mt-1">
                            Jarak: {validationResult.distance_meters.toFixed(2)}m
                            {validationResult.is_within_radius ? ' ✅ Dalam radius' : ' ❌ Di luar radius'}
                          </p>
                        </div>
                        <button
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      />
                      <p className="mt-1 text-xs text-gray-500">
                        Jumlah maksimal karyawan yang dapat bekerja di lokasi ini
                      </p>
                    </div>
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <input
                              ...
                              disabled={isSubmitting || loading}
                            />
                            <span className="absolute right-3 top-2.5 text-xs text-gray-500">-90 to 90</span>
                          </div>
                        </div>
                        <div>
                          ...
                          <div className="relative">
                            <input
                              ...
                              disabled={isSubmitting || loading}
                            />
                            <span className="absolute right-3 top-2.5 text-xs text-gray-500">-180 to 180</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button
                        ...
                        disabled={isSubmitting || loading || !formData.latitude || !formData.longitude}
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-sm font-medium text-gray-700">{formData.radius}m</span>
                      </div>
                      <input
                        type="range"
                        name="radius"
                      ...
                      <div className="flex justify-between text-xs text-gray-500 mt-1">
                        <span>50m</span>
                        <span>Jarak maksimal untuk check-in (50-300 meter)</span>
                        <span>300m</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mb-6">
                    <div className="flex items-center justify-between mb-3">
                      <h4 className="font-medium text-gray-900">Lokasi yang Telah Ada</h4>
                      {activeLocations.length > 3 && (
                        <button
                          onClick={() => setShowAllLocations(!showAllLocations)}
                          className="text-sm text-blue-600 hover:text-blue-800"
                        >
                          {showAllLocations ? 'Tampilkan Sedikit' : `Lihat Semua (${activeLocations.length})`}
                        </button>
                      )}
                    </div>
                            ...
                            <div className="flex items-start justify-between">
                              <div className="flex items-start space-x-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-gray-600 mt-1 truncate max-w-md">{location.address}</p>
                                  <div className="flex items-center gap-4 mt-2 text-xs text-gray-500">
                                    <span>Kapasitas: {location.capacity} orang</span>
